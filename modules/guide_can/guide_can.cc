...
void guide_Canbus::PublishChassisDetail() {
  ...
  if(Mode==WORK_MODE){
    ...
    if (i<msg_count && record_reader->ReadMessage(&message)) {
      ChassisDetail sensordata_history;
      rmsg.ParseFromString(message.content);
      rmsg.SerializeToArray(&sensordata_history,sizeof(sensordata_history));
      
      //AINFO<<sensordata.DebugString();

      //update from history
      sensordata.set_uwb_distance(SphereDis(sensordata.gps_longitude(),sensordata.gps_latitude(),
                                  sensordata_history.gps_longitude(),sensordata_history.gps_latitude()));
      sensordata.set_uwb_azimuth(SphereAzimuth(sensordata.gps_longitude(),sensordata.gps_latitude(),
                                  sensordata_history.gps_longitude(),sensordata_history.gps_latitude())
                                  - sensordata.gps_azimuth());

      chassis_detail_writer_->Write(sensordata);
      i++;
    }else{
      AERROR<<"No more Records.";
    }
  }
  return;
}
